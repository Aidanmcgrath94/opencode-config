...
The native \`task\` tool returns each subagent's findings inline once it completes — read those results directly from the main-session analysis and the explore agent calls above; there is no separate background-collection step.
