...
2. For each chunk, launch all `task` calls **in a single message** so they run in parallel.
3. Each `task` call blocks until its subagent finishes and returns its result inline — collect the 5 results directly from those tool returns.
