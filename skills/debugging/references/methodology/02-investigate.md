...
Fan out subagents via the native `task` tool. Same rule: one hypothesis per subagent. Issue all `task` calls **in a single message** so they run in parallel; each call blocks until its subagent finishes and returns its findings inline — read those results directly, then synthesize.

```
task(subagent_type="explore", load_skills=[],
     description="Investigate hypothesis 1",
     prompt="[CONTEXT: bug summary + which hypothesis you own + what state to look at]
     Runtime state investigation for hypothesis 1: ...")
task(subagent_type="explore", load_skills=[],
     description="Investigate hypothesis 2",
     prompt="Log/timing investigation for hypothesis 2: ...")
task(category="deep", load_skills=[],
     description="Investigate hypothesis 3",
