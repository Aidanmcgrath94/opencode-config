...
# PHASE 0.5: REVIEW CONVERSATION HISTORY FIRST (MANDATORY FIRST DATA STEP)

Before any other context gathering step, scroll back through the conversation history already present in your context window. That history is the only authoritative source for what the user originally asked. Do not reconstruct memory of the first request from a vague impression; if this session has been compacted or summarized, earlier messages may be truncated or lossy, so treat any such summary as unreliable.

From the conversation history:

1. Find the first user message in this session (the earliest turn with role "user").
...
Rules:
- Do not reconstruct user requests from a fuzzy memory. Always quote from the actual conversation turns.
...
- If the early conversation has been compacted/summarized and the original wording is unavailable, state that explicitly in the USER REQUESTS (AS-IS) section rather than guessing, and ask the user to confirm or restate their original request.
...
1. Conversation history (already reviewed in PHASE 0.5; reuse those findings here)
...
USER REQUESTS were already captured verbatim from the conversation history in PHASE 0.5; do not re-derive them here.
...
- USER REQUESTS (AS-IS) must come from the PHASE 0.5 conversation-history extraction, copied verbatim (do not paraphrase, do not reconstruct from a fuzzy memory)
