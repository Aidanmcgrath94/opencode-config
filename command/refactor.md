...
Fire ALL of these simultaneously by delegating to subagents via the native `task` tool — issue all five `task` calls in a single message so they run in parallel:
...
task(
  subagent_type="explore",
  description="Find dependents of target",
...
While the exploration subagents are running, use direct tools:
...
The native `task` tool blocks until each subagent finishes and returns its findings inline in the tool result — there is no separate background-collection step. Read the returned text from each of the five `task` calls above directly.
